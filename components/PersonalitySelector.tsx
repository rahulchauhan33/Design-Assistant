...
const PersonalitySelector: React.FC<PersonalitySelectorProps> = ({ activePersonality, onSelectPersonality, isLoading }) => {
  ...
  return (
    <div className="grid grid-cols-4 gap-1 sm:gap-2 w-full" role="tablist" aria-label="Select a personality">
      {PERSONALITIES.map((p, index) => {
        ...
        return (
          ...
          >
            <p.icon className={`h-6 w-6 sm:h-10 sm:w-10 mb-1 transition-colors ${isActive ? 'text-[#69961a] dark:text-[#ABF62D]' : 'text-gray-600 dark:text-gray-400'}`} aria-hidden="true" />
            <h3 className={`text-xs sm:text-base font-semibold transition-colors ${isActive ? 'text-gray-900 dark:text-white' : 'text-gray-800 dark:text-gray-300'}`}>{p.name}</h3>
